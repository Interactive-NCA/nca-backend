...
def simulate(
    ...
    for (n_episode, init_state) in enumerate(init_states):
        ...
        while not done:
            ...
            action, done = model(in_tensor, fixed_tiles=fixed_tiles)
...
#       else:
        ...
        self.gen_model = globals()[MODEL](
            ...
            map_dims=self.env.get_map_dims()[:-1],
            render=RENDER, n_aux_chan=args.n_aux_chan, binary_channel = BINARY_CHANNEL) 
        # TODO: toggle CUDA/GPU use with command line argument.

        # Save the model setup 
        model_init = {
            "n_in_chans": int(n_observed_tiles + N_LATENTS),
            "n_actions": int(n_out_chans),
            "map_width": int(self.env.unwrapped._prob._width),
            "map_dims": list(self.env.get_map_dims()[:-1]),
            "render": RENDER,
            "n_aux_chan": int(args.n_aux_chan),
            "binary_channel": BINARY_CHANNEL
        }
        with open(os.path.join(SAVE_PATH, "model_init.json"), "w") as f:
            json.dump(model_init, f)
    ...
    def evolve(self):
        ...
        for itr in tqdm(range(self.n_itr, self.total_itrs + 1)):
            ...
            # Generate random fixed tiles in every generation 
            fixed_tiles = []
            # TODO: NOW WORKS ONLY WITH ZELDA
            if RANDOM_FIXED_TILES != 0:
                for _ in range(RANDOM_FIXED_TILES):
                    tile_type = random.randint(0,8)
                    x = random.randint(0,15)
                    y = random.randint(0,15)
                    tile = [tile_type, x, y] # Tile type, x, y
                    fixed_tiles.append(tile)
            else:
                fixed_tiles=arg_dict["fixed_tiles"]["zelda"]
...
if __name__ == "__main__":
    ...
    BINARY_CHANNEL = arg_dict["binary_channel"]
    ...
    if arg_dict.get("random_fixed_tiles") is not None:
        RANDOM_FIXED_TILES = arg_dict.get("random_fixed_tiles")
    else:
        RANDOM_FIXED_TILES = 0
